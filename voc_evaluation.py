...
# Utility functions
import utils.inference as TRTInference
...
def analyze_tensorrt_prediction(detection_out, pred_start_idx):
    ...
    xmin = float(xmin) * DetectionModel.get_input_width()
    xmax = float(xmax) * DetectionModel.get_input_width()
    ymin = float(ymin) * DetectionModel.get_input_height()
    ymax = float(ymax) * DetectionModel.get_input_height()
    ...
    for idx in range(0, len(image_numbers), max_batch_size):
        ...
        for img_idx, img_number in enumerate(imgs):
            ...
            for det in range(int(keep_count[img_idx])):
                ...
                if confidence > 0.0:
                    label_name = voc.coco_label_to_voc_label(COCO_LABELS[label])
                    if label_name:
            ...
            if not os.path.exists(voc_ppm_path):
                ...
                img_pil = img_pil.resize(
                    size=(
                        ...
                        DetectionModel.get_input_height()),
